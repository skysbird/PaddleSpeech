...
def train_sp(args, config):
    ...
    dev_batch_size = min(len(dev_metadata), config.batch_size)

    print("llllll",len(train_metadata))
...
def main():
    ...
    config_file = pretrained_model_dir / "pwg_default.yaml"
    with open(config_file) as f:
        config = CfgNode(yaml.safe_load(f))

    config.train_max_steps = config.train_max_steps + args.epoch
